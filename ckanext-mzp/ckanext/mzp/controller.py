...
class SourceController(PackageController):
    ...
    def dataset_source_list(self, id):
        '''
        ...
        try:
            c.pkg_dict = get_action('package_show')(context, data_dict)
            c.source_list = get_action('package_source_list')(
        ...
        if request.method == 'POST':
            # Are we adding the source of dataset?
            new_source = request.POST.get('source_added')
            source_title = request.POST.get('title_added') or 'external link'
            log.info(new_source)
            if new_source:
                data_dict = {"source_link": new_source,
                             "package_id": c.pkg_dict['id'],
                             "source_title": source_title}
                try:
                    get_action('package_add_source')(
                        context, data_dict)
                except NotFound:
                    abort(404, _('Showcase not found'))
                else:
                    h.flash_success(
                        _("The source has been added to the dataset."))

            # Are we removing a source from a dataset?
            source_to_remove = request.POST.get('remove_source_id')
            if source_to_remove:
                data_dict = {"source_id": source_to_remove}
                try:
                    get_action('package_delete_source')(
                        context, data_dict)
                except NotFound:
                    abort(404, _('Source not found'))
                else:
                    h.flash_success(
                        _("The source has been removed from the dataset."))
            redirect(h.url_for(
                controller='ckanext.mzp.controller:SourceController',
                action='dataset_source_list', id=c.pkg_dict['name']))
        #
        # pkg_showcase_ids = [showcase['id'] for showcase in c.showcase_list]
        # site_showcases = get_action('ckanext_showcase_list')(context, {})
        #
        # c.package_dropdown = [[showcase['id'], showcase['title']]
        #                        for showcase in site_showcases
        #                        if showcase['id'] not in pkg_showcase_ids]

        log.info(c.pkg_dict)
        return render("package/dataset_source_list.html", extra_vars={'pkg_dict': c.pkg_dict})
